...
function selectOption(index) {
    // Target only the main options container, not child options
    const optionsContainer = document.getElementById('options-container');
    if (!optionsContainer) return;
    
    // Only select direct .option children that are NOT child-options
    const options = optionsContainer.querySelectorAll('.option:not(.child-option)');
    const radios = optionsContainer.querySelectorAll('.option:not(.child-option) .option-radio');
    ...
    if (userAnswers[currentQuestionIndex] === index) {
        ...
        options[index].classList.remove('selected', 'border-primary');
        options[index].classList.add('border-base-300');
    ...
    // Update question navigator to show answered state
    updateQuestionNavigator();
